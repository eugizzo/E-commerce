...
import { getAllCart } from '../reducer/cartSlice';
const Navbar = () => {
      ...
      <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
        <div className="relative flex h-16 items-center justify-between">
          <div className="absolute inset-y-0 left-0 flex items-center sm:hidden">
            {/* Mobile menu button*/}
            <button onClick={handleToggleOpen} className="flex items-center px-3 py-2 border-3 rounded text-white border-gray-500  hover:border-[#05C605] hover:text-[#05C605]">
            {isOpen && (<svg className="fill-current tex-white h-3 w-3" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>Menu</title><path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z" /></svg>

            )}
            {!isOpen && (
              <svg className="fill-current tex-white-300 h-3 w-3" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>Menu</title><path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z" /></svg>
            )}
          </button>
          </div>
        ...
        <div class="t-0 absolute left-3">
         <Link to="/cart" class="flex h-2 w-2 items-center justify-center rounded-full bg-red-500 p-3 text-xs text-white">  {cartCount.length}</Link>
        </div>
  ...
  <svg className="h-6 text-white w-6" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" aria-hidden="true">
